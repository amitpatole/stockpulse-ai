...
## Technical Design Spec: VO-400 — CSV Import for Watchlist Management
...
- Request: `multipart/form-data` with field `file` (`.csv`, max 1MB)
- Validates: file extension `.csv`, size ≤ 1MB, row count ≤ 500
...
```
**Errors:** `400` (bad file type, empty file, no symbol column), `404` (watchlist not found), `413` (file too large)
