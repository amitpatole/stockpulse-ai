...
# VO-353: CSV Import for Watchlist Management
...
- File size capped at 1 MB (enforced server-side via `MAX_CONTENT_LENGTH` before the body is read into memory, not via a post-read check); rows capped at 500 symbols
- CSV files must be UTF-8 encoded; non-UTF-8 files are rejected with a clear error message
...
**Security**
- Importing a CSV into a watchlist that belongs to another user is rejected with a `403 Forbidden` — ownership is verified before any data is read or written
- Symbol values beginning with `=`, `+`, `-`, or `@` are rejected as potential CSV injection payloads and reported in the invalid symbols list
