...
# Per-alert sound overrides: 'default' means "use the global sound setting"
_VALID_SOUND_TYPES = {"default", "chime", "alarm", "silent"}
# Global sound_type cannot be 'default' (that is a per-alert override value only)
_VALID_GLOBAL_SOUND_TYPES = {"chime", "alarm", "silent"}
...
def update_alert_sound_endpoint(alert_id: int):
    ...
    Returns:
        200 with { id, sound_type }, 400 on invalid input, 404 if not found.
    """
    data = request.get_json(silent=True) or {}
    ...
    updated = update_alert_sound(alert_id, sound_type)
    if updated is None:
        return jsonify({"error": f"Alert {alert_id} not found"}), 404
...
def get_sound_settings():
    ...
    """
    enabled = (
        get_setting("alert_sound_enabled", _SOUND_DEFAULTS["alert_sound_enabled"])
        == "true"
    )
    sound_type = get_setting("alert_sound_type", _SOUND_DEFAULTS["alert_sound_type"])
    volume = int(
        get_setting("alert_sound_volume", _SOUND_DEFAULTS["alert_sound_volume"])
    )
    mute_when_active = (
        get_setting(
            "alert_mute_when_active", _SOUND_DEFAULTS["alert_mute_when_active"]
        )
        == "true"
    )
    return jsonify(
...
def update_sound_settings():
    ...
    if "sound_type" in data:
        if data["sound_type"] not in _VALID_GLOBAL_SOUND_TYPES:
            return jsonify(
                {
                    "error": f"Invalid sound_type. Must be one of: {', '.join(sorted(_VALID_GLOBAL_SOUND_TYPES))}"
                }
            ), 400
    ...
    if "mute_when_active" in data:
        set_setting(
            "alert_mute_when_active", "true" if data["mute_when_active"] else "false"
        )
