...
# Serialises all multi-step write operations on the ai_providers table.
# Reads are left unlocked — SQLite WAL mode provides safe concurrent reads.
...
def set_active_provider(provider_id: int) -> bool:
    ...
    try:
        with _lock:
            ...
            # Verify the provider exists before mutating any state.
            # Without this check, a missing id would deactivate all providers
            # and leave the table with zero active rows.
            cursor.execute('SELECT id FROM ai_providers WHERE id = ?', (provider_id,))
            if cursor.fetchone() is None:
                conn.close()
                logger.warning(f"Provider {provider_id} not found; active state unchanged")
                return False
