...
function playAlertSound(settings: AlertSoundSettings, perAlertSoundType?: string): void {
  ...
  // Per-alert sound_type overrides global; 'default' falls back to global setting
  const rawType = perAlertSoundType && perAlertSoundType !== 'default'
    ? perAlertSoundType
    : settings.sound_type;

  if (rawType === 'silent') return;

  const audio = new Audio(`/sounds/${rawType}.mp3`);
