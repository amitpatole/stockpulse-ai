 ...
 * Network-level failures (offline, CORS, DNS) are caught and surfaced as ApiError.
 */
...
async function apiFetch<T>(path: string, init?: RequestInit, _attempt = 0): Promise<T> {
  ...
  try {
    ...
    const err = new ApiError(message, 0, 'NETWORK_ERROR');
    if (_errorReporter) _errorReporter(err);
    throw err;
  }
  ...
  if (!res.ok) {
    ...
    if (_RETRYABLE_STATUSES.has(res.status) && _attempt < _MAX_RETRIES) {
      ...
      const delaySec = retryAfterHeader ? parseInt(retryAfterHeader, 10) : (retryAfterBody ?? 1);
    ...
    if (isClientError && _errorReporter) _errorReporter(err);
...
export async function toggleAlert(alertId: number): Promise<PriceAlert> {
  return apiFetch<PriceAlert>(`/api/alerts/${alertId}/toggle`, { method: 'PUT' });
}
